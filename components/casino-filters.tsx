...
import { Star, CheckCircle2, ChevronDown, Landmark, CreditCard, Gamepad2, Gift, Sparkles } from "lucide-react"
...
const bonusTypeIcons: Record<string, string> = {
  ...
  "Free Credit Casinos": "/icons/free-bonus.svg", // Updated to use new free-bonus icon
...
type FilterCategory = "countries" | "payments" | "gameTypes" | "bonusTypes" | "providers"
...
export function CasinoFilters() {
  ...
  const [openFilters, setOpenFilters] = useState<Record<FilterCategory, boolean>>({
    countries: true,
    payments: true,
    gameTypes: true,
    bonusTypes: true,
  ...
  const filteredCasinos = useMemo(() => {
    return casinos.filter((casino) => {
      if (filters.countries.length > 0) {
        if (!filters.countries.some((c) => casino.countries.includes(c))) return false
      }
      if (filters.payments.length > 0) {
        if (!filters.payments.some((p) => casino.payments.includes(p))) return false
      }
      if (filters.gameTypes.length > 0) {
        if (!filters.gameTypes.some((g) => casino.gameTypes.includes(g))) return false
      }
      if (filters.bonusTypes.length > 0) {
        if (!filters.bonusTypes.some((b) => casino.bonusTypes.includes(b))) return false
      }
      if (filters.providers.length > 0) {
        if (!filters.providers.some((p) => casino.providers.includes(p))) return false
      }
      return true
    })
  }, [filters])
  ...
  const FilterSection = ({
    ...
    <Collapsible
      ...
      onOpenChange={(open) => setOpenFilters((prev) => ({ ...prev, [category]: open }))}
    >
      <CollapsibleTrigger className="w-full">
        <div className="flex items-center justify-between py-3 hover:text-primary transition-colors">
          ...
          <ChevronDown className={`w-4 h-4 transition-transform ${openFilters[category] ? "rotate-180" : ""}`} />
        </div>
      </CollapsibleTrigger>
        ...
        <div className="grid grid-cols-1 gap-2">
          {options.map((option) => (
            <button
              ...
              className={`w-full text-left px-3 py-3 rounded-lg text-sm transition-all flex items-center gap-3 relative overflow-hidden group ${
                filters[category].includes(option)
                  ? "bg-primary text-primary-foreground shadow-md scale-[1.02] border-2 border-primary"
            ...
            >
              <div
                ...
                style={{
                  background: "radial-gradient(circle at 30% 50%, rgba(184, 244, 0, 0.15) 0%, transparent 70%)",
                }}
              />
              ...
              {iconMap && (
                <span className="flex-shrink-0 w-10 h-10 flex items-center justify-center relative z-10">
                  {iconMap[option]?.startsWith("/icons/") ? (
                    ...
                    <span className="text-2xl drop-shadow-md">{iconMap[option]}</span>
                  )}
                </span>
              )}
              <span className="flex-1 relative z-10 font-medium">{option}</span>
              {filters[category].includes(option) && <CheckCircle2 className="w-5 h-5 flex-shrink-0 relative z-10" />}
            </button>
          ))}
        </div>
      </CollapsibleContent>
        ...
        <div className="space-y-4">
          {filteredCasinos.length === 0 ? (
            <Card className="p-12 text-center">
              <p className="text-muted-foreground">No casinos match your filters. Try adjusting your criteria.</p>
            </Card>
          ) : (
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div className="flex-shrink-0">
          <div className="w-32 h-32 bg-secondary rounded-lg flex items-center justify-center overflow-hidden">
            <img src={casino.logo || "/placeholder.svg"} alt={casino.name} className="w-full h-full object-cover" />
          </div>
        </div>
        ...
        <div className="flex-1 space-y-4">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            ...
            <Button className="bg-primary hover:bg-primary/90 text-primary-foreground">Visit Casino</Button>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <div className="text-muted-foreground">
                  ...
                  {casino.payments.length > 3 && ` +${casino.payments.length - 3}`}
                </div>
              </div>
              <div>
                ...
                <div className="text-muted-foreground">
                  ...
                  {casino.gameTypes.length > 2 && ` +${casino.gameTypes.length - 2}`}
                </div>
              </div>
              <div>
                ...
                <div className="text-muted-foreground">
                  ...
                  {casino.providers.length > 2 && ` +${casino.providers.length - 2}`}
                </div>
              </div>
              <div>
                ...
                <div className="text-muted-foreground">
                  ...
                  {casino.countries.length > 2 && ` +${casino.countries.length - 2}`}
                </div>
              </div>
            </div>
          </div>
